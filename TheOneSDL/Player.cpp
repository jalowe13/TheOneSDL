...
Player::Player(SDL_Renderer *renderer) : Entity(renderer) {
  ...
  while (tex_files.size() > 0) {
    ...
    // std::cout << "Player.cpp: Texture loaded " << name << " with path:" <<
...
Player::~Player() { std::cout << "-----Player Destroyed\n"; }

// Getters
const float Player::getSpeed() { return movementModifier; }

Entity::MovementDirection Player::xPath() { return currentDirectionX; }

Entity::MovementDirection Player::yPath() { return currentDirectionY; }

int Player::getX() { return playerR.x; }

int Player::getTexX() { return textureR.x; }

SDL_Texture *Player::getTexture() { return player_texture; }

int Player::getY() { return playerR.y; }

float Player::getMS() { return movementModifier; }

SDL_Rect *Player::getRect() { return &playerR; }

SDL_Rect *Player::getRectTex() { return &textureR; }
...
SDL_Rect *Player::getHitboxRect() { return &playerHitboxR; }

// Setters

void Player::xPathEdit(MovementDirection path) { currentDirectionX = path; }

void Player::yPathEdit(MovementDirection path) { currentDirectionY = path; }

void Player::xEdit(int x) { playerR.x = x; }

void Player::yEdit(int y) { playerR.y = y; }

void Player::wEdit(int w) { playerR.w = w; }

void Player::hEdit(int h) { playerR.h = h; }

void Player::xTexEdit(int x) { textureR.x = x; }

void Player::yTexEdit(int y) { textureR.y = y; }

void Player::wTexEdit(int w) { textureR.w = w; }

void Player::hTexEdit(int h) { textureR.h = h; }

void Player::editMS(float speed) { movementModifier = speed; }
